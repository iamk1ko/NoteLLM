...
class ChatMessage(Base):
    ...
    role: Mapped[str] = mapped_column(
        Enum("user", "assistant", "system", "tool", name="chat_role"), nullable=False, comment="消息角色"
    )
    content: Mapped[str] = mapped_column(Text, nullable=False, comment="消息内容")
    model_name: Mapped[str | None] = mapped_column(String(100), nullable=True, comment="使用的模型名称")
    token_count: Mapped[int | None] = mapped_column(Integer, nullable=True, comment="消息的Token数量")
