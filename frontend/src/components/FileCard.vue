...
const emit = defineEmits<{
  (e: 'click'): void; // 点击卡片整体 (通常跳转到详情)
  (e: 'delete', id: string): void; // 点击删除按钮
}>();
...
const statusClass = computed(() => {
  if (props.file.status === 1) return 'status-success'; // Assuming 1 is Ready
  if (props.file.status === 0) return 'status-processing'; // Assuming 0 is Processing
  return 'status-failed';
});

// 状态标签样式 (Pill Class)
const statusPillClass = computed(() => {
  if (props.file.status === 1) return 'pill--success';
...
const statusText = computed(() => {
  const map: Record<number, string> = {
    1: '就绪',
    0: '处理中',
    // failed: '错误' // Need mapping for failed status code
  };
...
.status-processing { background: #f59e0b; animation: blink 1s step-end infinite; }
